  ...
  return (
    <div className="card">
      <div className="card-title" style={{ justifyContent: "space-between" }}>
        ...
        >
          <span style={{ fontSize: 22, color: "#E24B4A" }}>{collapsed ? "▸" : "▾"}</span>
          🙋 ผู้เล่นวันนี้
        </span>
        {!collapsed && (
          <button
          ...
          >
            📇 {manageOpen ? "ซ่อนรายชื่อ" : "จัดการรายชื่อในคลัง"} ({state.roster.length})
          </button>
        )}
      </div>
      ...
      {!collapsed && (
        <>
          {manageOpen && (
            <div style={{ marginBottom: 12 }}>
              <div className="roster-add">
                <input
                  type="text"
                  className="roster-add-input-purple"
                  placeholder="เพิ่มชื่อผู้เล่นในคลัง..."
                  maxLength={30}
                  value={newName}
          ...
          <div className="search-dropdown-wrap" ref={pickerWrapRef}>
            ...
            {pickerOpen && (
              <div className="search-dropdown-list">
                ...
                {state.roster.length === 0 ? (
                  <div className="search-dropdown-empty">ยังไม่มีชื่อผู้เล่นในคลัง</div>
                ) : notPresentPlayers.length === 0 ? (
          ...
          <div className="roster-add">
            <input
              type="text"
              placeholder="เพิ่มชื่อผู้เล่นทั่วไป (ไม่บันทึกในคลัง)..."
              maxLength={30}
              value={guestName}
          ...
          {state.roster.length === 0 && (
            <div className="roster-empty">ยังไม่มีชื่อในคลัง เพิ่มได้จากปุ่ม &quot;จัดการรายชื่อในคลัง&quot; ด้านบน</div>
          )}
