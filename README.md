...
### ขั้นตอนที่ 5 — สร้างรหัสผ่านเจ้าของแอป (สำคัญ ห้ามข้าม)

migration ข้างต้นสร้างแค่ตารางเปล่าๆ **ไม่มีรหัสผ่านติดมาด้วยเลย** — ถ้าข้ามขั้นตอนนี้จะไม่มีใครล็อกอินเข้าแอปได้แม้แต่ตัวเจ้าของเอง (หน้าล็อกอินต้องมีรหัสอยู่ก่อน และการออกรหัสใหม่ก็ต้องล็อกอินเป็นเจ้าของแอปก่อนถึงจะทำได้ — ติดเป็นวงกลม)

เปิด **Neon Console → SQL Editor** แล้ววางคำสั่งนี้ (แก้ `MYCODE01` เป็นรหัส 8 ตัวที่อยากใช้เอง):

```sql
INSERT INTO "AccessCode" (code, label, "isOwner") VALUES ('MYCODE01', 'เจ้าของแอป', true);
```

เสร็จแล้วเปิดลิงก์ Vercel ที่ได้ แล้วกรอกรหัสนี้เข้าไปเป็นเจ้าของแอป จากนั้นไปออกรหัสอื่นให้เพื่อนๆ ผ่านหน้า ⚙️ ตั้งค่า ได้ตามปกติ — ทุกคนที่เปิดลิงก์นี้จะเห็นข้อมูลชุดเดียวกัน
